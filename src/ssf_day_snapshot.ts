...
// Up to a quarter per request; D1 allows 100 bound parameters per query (poolId + one per date)
export const MAX_DATES = 92;
...
export async function daySnapshot(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
	...
	let rowsByDate: Record<string, any[]>;
	try {
		({ rowsByDate } = await getDayRows(env, ctx, poolId, dates, refresh));
	} catch (error) {
...
// Latest-scrape rows of each date (yyyy-mm-dd), from KV when cached, else from D1.
// settledDates holds the dates whose rows are final (served from cache, or cached now).
export async function getDayRows(env: any, ctx: ExecutionContext, poolId: string, dates: string[], refresh = false)
	: Promise<{ rowsByDate: Record<string, any[]>, settledDates: Set<string> }> {
	...
	if (!refresh) {
		...
		dates.forEach((d, i) => {
			if (cached[i] !== null) {
				rowsByDate[d] = JSON.parse(cached[i]!);
				settledDates.add(d);
			}
		});
	}
...
ORDER BY SESSION_DATETIME, SESSION_SIDE, SESSION_TITLE
		`;
		const { results } = await env.DB.prepare(query).bind(poolId, ...missedDates.map(toSessionDate)).all();
		...
		for (const date of missedDates) {
			...
			const rows = results.filter((r: any) => r.SESSION_DATE === sessionDate);
	...
	return { rowsByDate, settledDates };
}
